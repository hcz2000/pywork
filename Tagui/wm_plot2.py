...
class QtDemo(QWidget):
  ...
  def __init__(self, products,parent=None):
    ...
    rightMiddleLayout1=QVBoxLayout()
    rightMiddleLayout2=QVBoxLayout()
    rightLowerLayout1=QVBoxLayout()
    rightLowerLayout2=QVBoxLayout()
    rightMiddleFrame1 = QFrame()
    rightMiddleFrame2 = QFrame()
    rightLowerFrame1 = QFrame()
    rightLowerFrame2 = QFrame()
    rightMiddleFrame1.setLayout(rightMiddleLayout1)
    rightMiddleFrame2.setLayout(rightMiddleLayout2)
    rightMiddleFrame1.setMaximumHeight(math.floor(resolution.height() * 0.45))
    rightMiddleFrame2.setMaximumHeight(math.floor(resolution.height() * 0.45))
    rightLowerFrame1.setLayout(rightLowerLayout1)
    rightLowerFrame2.setLayout(rightLowerLayout2)
    rightLowerFrame1.setMaximumHeight(math.floor(resolution.height() * 0.45))
    rightLowerFrame2.setMaximumHeight(math.floor(resolution.height() * 0.45))

    rightMiddleSplitter= QSplitter(Qt.Orientation.Horizontal)
    rightMiddleSplitter.addWidget(rightMiddleFrame1)
    rightMiddleSplitter.addWidget(rightMiddleFrame2)

    rightLowerSplitter= QSplitter(Qt.Orientation.Horizontal)
    rightLowerSplitter.addWidget(rightLowerFrame1)
    rightLowerSplitter.addWidget(rightLowerFrame2)

    right_splitter = QSplitter(Qt.Orientation.Vertical)
    right_splitter.addWidget(rightUpperFrame)
    right_splitter.addWidget(rightMiddleSplitter)
    right_splitter.addWidget(rightLowerSplitter)

    self.upperFig1=plt.Figure()
    self.upperCanvas1=FC(self.upperFig1)
    self.upperFig2=plt.Figure()
    self.upperCanvas2=FC(self.upperFig2)
    self.lowerFig1=plt.Figure()
    self.lowerCanvas1=FC(self.lowerFig1)
    self.lowerFig2=plt.Figure()
    self.lowerCanvas2=FC(self.lowerFig2)
    rightMiddleLayout1.addWidget(self.upperCanvas1)
    rightMiddleLayout2.addWidget(self.upperCanvas2)
    rightLowerLayout1.addWidget(self.lowerCanvas1)
    rightLowerLayout2.addWidget(self.lowerCanvas2)
  ...
  def draw(self):
    self.upperFig1.clear()
    ax1a = self.upperFig1.add_subplot(111)
    self.draw_subplot(ax1a, (datetime.now() - timedelta(days=365 + 1)).date())
    self.upperCanvas1.draw()

    self.upperFig2.clear()
    ax1b = self.upperFig2.add_subplot(111)
    self.draw_subplot(ax1b, (datetime.now() - timedelta(days=90 + 1)).date())
    self.upperCanvas2.draw()

    self.lowerFig1.clear()
    ax2a = self.lowerFig1.add_subplot(111)
    self.draw_subplot(ax2a, (datetime.now() - timedelta(days=60 + 1)).date())
    self.lowerCanvas1.draw()

    self.lowerFig2.clear()
    ax2b = self.lowerFig2.add_subplot(111)
    self.draw_subplot(ax2b, (datetime.now() - timedelta(days=30 + 1)).date())
    self.lowerCanvas2.draw()
  ...
  def draw_subplot(self, ax, start_date):
    if (datetime.now() - timedelta(days=60)).date() <= start_date:
      ...
      if (datetime.now() - timedelta(days=180)).date() <= start_date:
